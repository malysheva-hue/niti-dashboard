...
ACTIONS = {
    ...
    # ----- v2.1: высокая оборачиваемость + сниженная цена -----
    "high_turnover.raise_price": {
        "templates": [
            "Товар уходит за {days} дней — спрос держит. Цена снижена на {pct}% за неделю. Можно попробовать вернуть цену или поднять на 3-5% и посмотреть как ляжет конверсия за 5-7 дней.",
        ],
        "param_calc": {
            "days": lambda sku, f: round(sku.get("month", {}).get("turnover") or 0),
            "pct": lambda sku, f: 5,
...
VARIANTS_BY_RULE = {
    ...
    "HIGH_TURNOVER_LOW_PRICE": [
        "Поднять цену на 3-5% и наблюдать конверсию 5-7 дней",
        "Если конверсия не упадёт — вернуть прежнюю цену (нашу скидку убрать)",
        "Параллельно проверить остатки — спрос может вырасти быстрее поставки",
    ],
    "STAGNATION": [
